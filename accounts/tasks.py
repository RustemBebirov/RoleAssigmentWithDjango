...
from accounts.froms import UserForm
...
def send_confirmation_mail(user_id, site_address): 
    ...
    subject = _('Please confirm change your email') 
    context = { 'site_address': site_address, 'user': user, 'uid': urlsafe_base64_encode(force_bytes(user.id)), 'token': account_activation_token.make_token(user) } 
